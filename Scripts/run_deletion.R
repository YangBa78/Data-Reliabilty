...
p = glmer(as.factor(decision) ~ (1 | workerid) + (1 | task/imageid) + (1 | imageid:workerid) + (1 | task:workerid),   
           data = Prolific, family = binomial, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6)), nAGQ = 1)
summary(p)

p = glmer(as.factor(decision) ~ task + (1 | workerid) + (1 | imageid) + (1 | task:workerid),   
          data = Prolific, family = binomial, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6)), nAGQ = 1)
summary(p)
...
for (x in unique(simulated_df$imageid)) {
  ...
  varint.image <- append(varint.image , data.frame(VarCorr(p_temp))$vcov[1] + data.frame(VarCorr(p_temp))$vcov[2] + data.frame(VarCorr(p_temp))$vcov[3])
  ...
  varint.image.change <- append(varint.image.change, (data.frame(VarCorr(p_temp))$vcov[1] + data.frame(VarCorr(p_temp))$vcov[2] + data.frame(VarCorr(p_temp))$vcov[3]) /sum(data.frame(VarCorr(p_temp))$vcov) - p.var.imageint)
  ...
  disagreerate <- append(disagreerate, min(table(simulated_df[simulated_df$workerid==x, ]$decision))/sum(table(simulated_df[simulated_df$workerid==x, ]$decision)))
  p_value <- append(p_value, pchisq(-2*(logLik(p)[1]- logLik(p_temp)[1]), df=24, lower.tail=F))
  }
...
min(df.p$loglik[df.p$p_value<=0.05])
plot(df.p$loglik)
abline(h =  36.839, col = "red")
