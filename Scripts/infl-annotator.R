...
colnames(Prolific)[13] <- 'truelabel'
colnames(Prolific)[18] <- 'finaldecision'
Prolific$decision <- ifelse(Prolific$finaldecision=='different', 1, 0)
Prolific$true <- ifelse(Prolific$truelabel=='different', 1, 0)
...
p = glmer(decision ~ (1 | workerid) + (1 | task/imageid) + (1 | imageid:workerid) + (1 | task:workerid),     
          data = plf, family = binomial, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6)), nAGQ = 1)
summary(p)
...
# generate new data
plf = Prolific[, c('workerid', 'task', 'imageid', 'decision')]
plf = plf[plf$workerid !='2f33f23f23', ]

workerid = unique(plf$workerid)
task = rep('difficult', 138)
imageid = rep('add2', 138)
#decision = rep(NA, 138)

df_ <- data.frame(workerid, task, imageid)

df_$predicted <- predict (p, newdata=df_, type="response", allow.new.levels=T)
df_$decision <- simulate(p, seed=1, newdata=df_, re.form=NA,
                        allow.new.levels=T)$sim_1
df_$decision <- simulate(p, seed=1, newdata=df_, re.form=NULL,
                         allow.new.levels=T)$sim_1
...
p.var.image = data.frame(VarCorr(p))$vcov[4] / p.t
...
for (x in unique(Prolific$workerid)) {
  ...
  #varint.image <- append(varint.image , data.frame(VarCorr(p_temp))$vcov[1] + data.frame(VarCorr(p_temp))$vcov[4])
  ...
  #varint.image.change <- append(varint.image.change, (data.frame(VarCorr(p_temp))$vcov[1] + data.frame(VarCorr(p_temp))$vcov[4]) /sum(data.frame(VarCorr(p_temp))$vcov) - p.var.imageint)
  ...
  accuracy <- append(accuracy, sum(diag(table(Prolific[Prolific$workerid==x, ]$finaldecision, Prolific[Prolific$workerid==x, ]$truelabel)))/sum(table(Prolific[Prolific$workerid==x, ]$finaldecision, Prolific[Prolific$workerid==x, ]$truelabel)))
}
...
names(Data.p) <- c('imageid', 'loglik', 'var.image', 'varint.image', 'total.var','var.image.change', 
                   'varint.image.change', 'total.var.change', 'vartrace', 'vartrace2', 'vartrace3', 'accuracy')



df.p <- as.data.frame(Data.p)
View(df.p)
which.max(df.p$accuracy)
df.p$accuracy <- ifelse(df.p$accuracy ==1, 0, df.p$accuracy)


df.p1 = subset (df.p, select = -imageid)
cor(df.p1)
...
# correlation
cor(df.p$loglik, df.p$accuracy)
cor(df.p$trace, df.p$accuracy)
cor(df.p$varimageratio, df.p$accuracy)
cor(df.p$varimageintratio, df.p$accuracy)
...
# plot 
ggplot(data = df.p, aes(x = disagreerate, y = var.image.change)) +
  stat_poly_line() +
...
library("writexl")
write_xlsx(df.p,"E:/Dropbox (ASU)/Code/8.3\\df.p-annotator.xlsx")
