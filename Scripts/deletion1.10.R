...
df.p[, c('workerid', 'loglik')]
df.p[order(df.p$loglik, decreasing = TRUE), ]
...
plot(df.p$loglik, df.p$p_value)


# concatenate
dfl = df.p[, c('workerid', 'loglik')]%>% left_join(dtWorker, by = "workerid")
dfl

cor(dfl$loglik, abs(dfl$worker_eff))
...
ggplot(data = df.p, aes(x = workerid, y = loglik)) +
  stat_poly_line() +
